...
export async function POST(request: NextRequest) {
  try {
    ...
    if (conversation_id) {
      ...
      if (userMessageError) {
        return new Response(JSON.stringify({ error: "Could not save user message" }), {
          status: 500,
          headers: { "Content-Type": "application/json" },
        });
      }
    ...
    const readable = new ReadableStream({
      async start(controller) {
        ...
        const saveAssistantMessage = async () => {
          if (!conversation_id || !fullResponse.trim()) return;
          ...
          if (assistantMessageError) {
            throw new Error("Could not save assistant message");
          }
        };
